...
double[][] matrizPatrones=new double[10][35];//Array temporal con los patrones
String[] NombrePatrones= new String[10];//Array String con los nombres de los patrones
double[][] MatrizObjetivos=new double[10][10];//Array con los valores objetivo
...
int pos=0;//variable para inicializar la matriz objetivo
        for (int i=0;i<10;i++){
                for (int j=0;j<10;j++){
                if(j==pos){
                    MatrizObjetivos[i][j]=1;}
                else{
                 MatrizObjetivos[i][j]=0;
                }
 ...
 *****************************************************************************/
  ...
  zona.repaint();
    ...
    for (int j=0;j<35;j++)matrizPatrones[contPatrones][j]=matriz[j];
    NombrePatrones[contPatrones]=textField1.getText();//Se guarda el nombre del patrón
    ...
    //Se pasa el valor del patrón capturado a String para visualizarlo en debug
        ...
        debug.append("Nombre: \""+NombrePatrones[contPatrones]+"\" valor:\n");
        debug.append("["+ matS +"]\n");
        ...
        if (contPatrones==10){
                ...
                patronesM = new Matriz(matrizPatrones);//Se inicializa la matriz de patrones
                patronesM = Matriz.transponer(patronesM);//Se transpone => 35x10
                objetivosM= new Matriz(MatrizObjetivos);//Se inicializa la matriz objetivos
    ...
    private void calcularJActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_calcularJActionPerformed
        ...
        for (int i=0;i<10;i++){
        debug.append(NombrePatrones[i]+": "+String.valueOf(df.format(Matriz.transponer(resp).toArray()[0][i]))+"  ");
        }
        debug.append("\n");
        ...
        respuesta.setText(Matriz.masProbable(tmp, NombrePatrones));
