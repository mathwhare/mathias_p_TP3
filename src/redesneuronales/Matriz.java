...
for (int i=0; i<m.getFil()-5;i++){//eliminar -5
        for (int j=0; j<m.getCol()-5;j++){//eliminar -5
    ...
    for (int j=0; j<mB.getCol();j++){
                tmp=0;
                  for (int k=0; k<mA.getCol();k++){
                   ...
                   tmp+=mA.getFC(i,k)*mB.getFC(k, j);//ordenar a k, j

                }
                ret.setFC(i, j, tmp);//ordenar a i, j
        }
}
return ret;
}
...
double tmp=resp.toArray()[0][0];
    for (int i=1;i<resp.toArray()[0].length;i++){
            ...
            if(resp.toArray()[0][i]>tmp){// cambiar por el signo mayor
